...
// /kr/kospi — KOSPI 종목만 필터링한 시세 list (ADR-0014 메뉴 split).
// registry.market === "KOSPI" 인 종목 (20개) — 기존 QuoteTable + sparkline 재사용.
// 시장 지수 자체 (코스피 2,500 값) 는 Phase 1.5 — KIS 지수 endpoint 통합 후.

export const revalidate = 300;

export const metadata = {
  title: "코스피 (KOSPI) 시세 · 차트",
  description:
    "한국 KOSPI 종목 시세 — 삼성전자 · SK하이닉스 · 현대차 등 대형주. 일봉 차트 + 백테스트 진입.",
};
...
export default async function KospiPage({params}: Props) {
  const {locale} = await params;
  const tDisc = await getTranslations("disclaimer");
  const tSidebar = await getTranslations("sidebar.items");

  const kospiSymbols = krRegistry
    .filter((e) => e.market === "KOSPI")
    .map((e) => e.symbol);
  ...
  return (
    <main className="mx-auto w-full max-w-6xl flex-1 px-6 py-10">
      <header className="mb-6 flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-wider text-fg-subtle">KOSPI</p>
          <h1 className="text-2xl font-semibold tracking-tight text-fg sm:text-3xl">
            코스피
          </h1>
          <p className="mt-1 text-sm text-fg-muted">
            {tSidebar("quotes")} · KOSPI 대형주 {kospiSymbols.length}종목 · KRW
          </p>
        </div>
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: assetListJsonLd({
              ...
              quotes,
              nameMap,
              listName: "KOSPI 시세 (한국거래소 유가증권시장)",
            }),
          }}
        />
      )}
