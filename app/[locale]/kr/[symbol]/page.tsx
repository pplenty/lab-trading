      ...
      <header className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div className="flex flex-col gap-1">
          ...
          <p className="flex items-baseline gap-2 text-xs text-fg-subtle">
            ...
            {(() => {
              const s = getSector("kr", entry.symbol);
      ...
      {series && series.candles.length > 0 && (
        <section className="mb-6 rounded-lg border border-line bg-surface/30 p-3">
          <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              <TimeframeToggle basePath={`/kr/${entry.symbol}`} current={tf} range={range} />
              <ChartRangeToggle basePath={`/kr/${entry.symbol}`} current={range} tf={tf} />
            </div>
          </div>
